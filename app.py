...
<style>
  ...
  .person { background: #101014; border-left: 2px solid #4ADE80;
...
with st.sidebar:
    ...
    use_linkedin = st.checkbox("Find contacts on LinkedIn", value=False,
                               help="Searches for talent acquisition roles at "
                                    "this company. About $0.10 per run.")

    if st.button("Run pipeline", use_container_width=True) and new_domain.strip():
        d = new_domain.strip()
        # LinkedIn runs after triggers, because it reuses the company name
        # triggers.py extracted - "Razorpay" matches better than "razorpay.com".
        steps = [("Scraping site", "scrape.py")]
        if use_news:
            steps.append(("Searching news", "news.py"))
        steps.append(("Extracting triggers", "triggers.py"))
        if use_linkedin:
            steps.append(("Finding contacts", "linkedin.py"))
        steps.append(("Writing messages", "generate.py"))
...
<div class="hero">
  ...
  <div class="lede">
    ...
    <strong>verifiable triggers</strong>, finds who to send to, and writes
    three emails and three LinkedIn messages built on them. Every claim is
    checked back against what was actually found, and every source is linked
    so you can open it and check the work yourself.
  </div>
</div>
""", unsafe_allow_html=True)
...
# Who to send it to. The search is scoped by title AND company, so these are
# actual recruiters rather than a random sample of employees.
...
if li_file.exists():
    ...
    total = ldata.get("total_matching", 0)
    if people:
        label = f"Who to contact · {len(people)}"
        if total:
            label += f" of {total} at this company"
        with st.expander(label):
            st.caption(f'LinkedIn search: "{ldata.get("query", "")}" · '
                       f'longest tenure first')
            for p in people:
                bits = []
                if p.get("years_at_company"):
                    bits.append(f"{p['years_at_company']} years")
                if p.get("location"):
                    bits.append(p["location"])
                sub = " · ".join([p["role"]] + bits)
                st.markdown(
                    f'<div class="person">'
                    f'<div class="person-name">{p["name"]}</div>'
                    f'<div class="person-role">{sub}</div>'
